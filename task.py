...
class utils():
    """
    This class contains utility functions for data processing, correlation analysis, and visualization.
    Methods:
        spearman_correlation(df): Calculate the Spearman correlation and return the correlation matrix.
        random_forest_regressor(X, y): Perform Random Forest regression and return the sorted feature importance.
        flag_anomalies(df, correlation_matrix): Flag anomalies in the dataset based on correlation matrix.
        prepare_data(): Merge and clean the datasets, returning a pivoted DataFrame and target variables.
        plot_res(res_matrix, title, xlabel): Plot the results as a bar chart.
    """
    ...
    def random_forest_regressor(self, X, y):
        """
    ...
    def flag_anomalies(self, df, correlation_matrix):
        """
    ...
    def plot_res(self, res_matrix, title, xlabel):
        """
...
class TaskController(utils):
    """
    Class to help the execution of the tasks using the utils class.

    Methods:
    GlobalAndRegionalTrneds(df): Plot global and regional trends of PM2.5 levels.
    SpearmanCorrelation(df, target_variables): Perform Spearman correlation analysis and plot results.
    RandomForestRegressor(df, target_variables): Perform Random Forest regression analysis and plot results.
    DetectAnomalies(df): Detect anomalies in the dataset based on correlation matrix.
    main(): Main function to execute the tasks.
    """
    def GlobalAndRegionalTrneds(self, df):
        """
        Plotting the global and regional trends of PM2.5 levels.
        Parameters:
        df (pd.DataFrame): The DataFrame containing data to be processed.
        Returns:
        ...
        self.plot_res(top_bottom_10, title='Top 10 and Bottom 10 Countries by Mean PM2.5 Level', xlabel='Mean PM2.5 Level')
        plt.show()
    ...
    def SpearmanCorrelation(self, df, target_variables):
        """
        Perform Spearman correlation analysis on the dataset and plot the results.
        ...
        """
        df_spearman = df.copy()
        non_numeric_data = df_spearman.select_dtypes(exclude=['number']).columns
        corr_matrix = self.spearman_correlation(df_spearman)
        ...
        for target in target_variables:
            ...
            print("Top 10\n", top_10, "\n")
            self.plot_res(top_10, f"Top 10 correlations to {target}", xlabel)
            bottom_10 = sorted_selected_columns.tail(10)
            bottom_10_reversed = bottom_10.iloc[::-1]
            print("Bottom 10\n", bottom_10, "\n")
            self.plot_res(bottom_10, f"Bottom 10 correlations to {target}", xlabel)
    ...
    def RandomForestRegressor(self, df, target_variables):
        """
        Perform Random Forest regression analysis on the dataset and plot the results.
        Parameters:
        df (pd.DataFrame): The DataFrame containing data to be processed.
        Returns:
        None: Displays the plots.
        """
        df_RFR = df.copy()
        non_numeric_data = df_RFR.select_dtypes(exclude=['number']).columns
        ...
        for target in target_variables:
            y = df_RFR[target]
            sorted_importance = self.random_forest_regressor(X, y)
            print(f"##################################  Feature Importance for {target}")
            top_10 = sorted_importance.head(10)
            print("Top 10 Features:\n", top_10, "\n")
            self.plot_res(top_10, f"Top 10 importance to {target}", xlabel)

            bottom_10 = sorted_importance.tail(10)
            bottom_10_reversed = bottom_10.iloc[::-1]
            print("Bottom 10 Features\n", bottom_10, "\n")
            self.plot_res(bottom_10_reversed, f"Bottom 10 importance to {target}", xlabel)

    def DetectAnomalies(self, df):
        """
        Detect anomalies in the dataset based on correlation matrix.
        Parameters:
        df (pd.DataFrame): The DataFrame containing data to be processed.
        Returns:
        None: Displays the anomalies detected.
        """
        df_anomalies = df.copy()
        print("Finding anomalies...")
        bins = [0, 10, 15, 25, 35, 100]
        labels = ['very low', 'low', 'medium', 'high', 'very high']
        df_anomalies['categorised PM2.5 level'] = pd.cut(df_anomalies['Mean PM2.5 Level'], bins=bins, labels=labels)
        
        flags = self.flag_anomalies(df_anomalies, self.correlation_matrix_for_anomalie_detection)
        if flags == []:
    ...
    def main(self):
        ...
        prepared_df, target_variables = self.prepare_data()
        ...
        self.GlobalAndRegionalTrneds(prepared_df)
        breakpoint()
        ...
        self.SpearmanCorrelation(prepared_df, target_variables)
        breakpoint()
        ...
        self.RandomForestRegressor(prepared_df, target_variables)
        breakpoint()
        ...
        self.DetectAnomalies(prepared_df)
        breakpoint()
